...
int storeMenu()//MENU GIVES USER A LIST OF OPTIONS FOR OPERATING DRUGSTORE
{
	...
	printf("5 \t\t Display customer transactions\n\n\t ");
	printf("6 \t\t Exit menu\n\n\n");
...
void createFileCustomer()//CREATES A NEW CUSTOMER FILE
{
	...
	printf("Create new customer file?\n");
	printf("   1. Yes\n");
	printf("   2. No\n");
	printf("Enter corresponding number---->");
	scanf("%d",&ans);
...
void stockManagement()//STORE MANAGEMENT
{
	...
	int funcAns,answer,funcAns2,answer2,answer3,val,identification,amount;
	...
	while(funcAns!=7)
	{
		...
		if(funcAns==3)//ALLOWS USER TO UPDATE THE INFORMATION OF A DRUG
		{
			...
			if(drug.ID!=0)
		    {
				...
				while(funcAns2!=7)
				{
					...
					{
						printf("Drug sales---->$%.2f\n",drug.sales);
						printf("Are you sure that you want to clear this drug's sales?\n");
						printf("   1. Yes\n");
						printf("   2. No\n");
						printf("Enter corresponding number---->");
						scanf("%d",&answer3);
						...
						{
							drug.sales=0;
							printf("SUCCESS!! DRUG'S SALES HAVE BEEN RESETTED TO $%.2f\n",drug.sales);
						}/*end if*/
						else
						{
		...
		if(funcAns==4)//DELETES AN DRUG RECORD
		{
			...
			printf("\n\n");
			printf("Enter drug ID to delete(4 DIGITS)--->");
		    scanf("%d",&identification);         
			...
			printf("\n\n");
		    printf("**************************************\n");
	    	printf("****END OF DELETE AN DRUG FUNCTION****\n");
	    	printf("**************************************\n");
			printf("\n\n");
		...
		if(funcAns==5)//ALLOWS USER TO ADD STOCK TO AN DRUG
		{
			if ((inDataFile1 = fopen("myfiledrugs", "rb+"))==NULL)
		    {
		        printf("Cannot open drug file \n");
			...
			if(drug.ID!=0)
			{
			    ...
			    printf("Adding %d more stock to drug %s. Is this okay?\n",amount,drug.name);
			    printf("   1. Yes\n");
				printf("   2. No\n");
				printf("Enter corresponding number---->");
				scanf("%d",&val);
		...
		if(funcAns==6)//ALLOWS USER TO DISPLAY STOCK
		{
			if ((inDataFile1 = fopen("myfiledrugs", "rb+"))==NULL)
		    {
		        printf("Cannot open drug file \n");
...
void storeManagement()//STORE MANAGEMENT
{
	FILE*inDataFile1;//POINTER TO DRUGSTORE FILE
	FILE*inDataFile2;//POINTER TO CUSTOMER FILE
	int funcAns,val2,identification,num;
	float cost;
	...
	while(funcAns!=6)
	{
		if(funcAns==1)//SELL DRUGS
		{
			...
			printf("\n\n");
			printf("Enter ID of drug (-1 to terminate)--->");
			scanf("%d",&identification);
		...
		{		  	      
			...
			printf("\n\n");					
			printf("Enter customer first name to search--->");
			scanf("%s",&string);
			...
			{   
			    ...
			    if(stricmp(customer.firstname,string)==0)
			    {
			        printf("\n\n");
			        printf("***Hernando Sanchez Pharmacy***\n");
			        printf("Coles Pasture, St. Philip, Barbados\n");
					printf("Cust ID   ");						
		...
		if(funcAns==4)//DELETE A TRANSACTION
		{
			if((inDataFile2 = fopen("myfilecustomers", "rb+"))==NULL)
			{
		        printf("Cannot open customer file \n");
			...
			printf("\n\n");
			printf("Enter customer ID to delete transaction(4 DIGITS)--->");
		    scanf("%d",&identification);         
		...
		if(funcAns==5)//DISPLAY CUSTOMER TRANSACTIONS
		{
			if((inDataFile2 = fopen("myfilecustomers", "rb+"))==NULL)
			{
		        printf("Cannot open customer file \n");
