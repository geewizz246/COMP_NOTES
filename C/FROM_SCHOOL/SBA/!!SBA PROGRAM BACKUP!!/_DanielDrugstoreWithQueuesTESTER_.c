...
void stockFormat(FILE*drugFile,struct drugInfo drug)
{
	drugFile = fopen("myfiledrugs", "rb+");/*open stock file*/
...
void orderFormat(FILE*orderFile,struct orderInfo order)
{
	int i;
...
void queueFormat(FILE*queueFile,struct queueInfo queue)
{
	int i;
...
void dequeueOrder(FILE*pointer1,struct queueInfo queue)
{
	...
	else
	{
		...
		if(queue.position!=0)/*if record exists*/
		{
			...
			printf("======= ORDER WITH ID %d HAS BEEN FULFILLED =======\n\n\n",val);
...
void deleteOrder(int identification,FILE*pointer1,FILE*pointer2)
{
	...
	fread(&order,sizeof(struct orderInfo),1,pointer1);/*Read Mode*/
	...
	{	
		...
		{
			fseek(pointer2,(queue.position)*sizeof(struct queueInfo),SEEK_SET);
			fread(&queue,sizeof(struct queueInfo),1,pointer2);
			if(queue.ID==identification)/*if record is found in queue*/
			{
				int dltPt=queue.position-1;/*records position of record to delete*/
				val=queue.ID;/*takes ID of customer in queue*/
...
//READS AND DISPLAYS ORDERS FROM THE ORDER FILE
void viewOrders()
{
	...
	{   
		...
		if(order.ID!=0)
		{
			orderFormat(orderFile,order);/*call to function orderFormat*/
		}/*end if*/
	}/*end while*/
...
{
	FILE*drugFile;//POINTER TO STOCK FILE
	int funcAns,val,answer,funcAns2,identification,number;
	float low,high;
	...
	while(funcAns!=6)
	{
		...
		if(funcAns==4)//ALLOWS USER TO UPDATE THE INFORMATION OF A DRUG
		{
			...
			if(drug.ID==identification)/*drug was found*/
			{
				stockFormat(drugFile,drug);/*call to function stockFormat*/
...
void pharmacyManagement()
{
	...
	funcAns=pharmacyMenu();/*call to function pharmacyMenu*/
	...
	while(funcAns!=8)
	{
		if(funcAns==1)//DISPLAY ORDERS IN QUEUE
		{
			...
			{  
				...
				if(queue.position!=0)/*if record exists*/
				{
					queueFormat(queueFile,queue);/*call to function queueFormat*/
				}/*end if*/		
			}/*end while*/
		...
		{
			viewOrders();/*call to function that viewOrders*/
		...
		if(funcAns==5)//DELETE AN ORDER
		{
			...
			if(identification!=-1)
			{
				deleteOrder(identification,orderFile,queueFile);/*call to function deleteOrder*/
			}/*end if*/
			else
			{
				printf("\n\nDelete cancelled.\n\n\n");
		...
		if(funcAns==2)//RECEIVE ORDER
		{
			...
			else
			{
				...
				if(stricmp(confirmOrder,"yes")==0)//YES
				{
					...
					queueFormat(queueFile,queue);/*call to function queueFormat*/
					dequeueOrder(queueFile,queue);/*call to function dequeueOrder*/
				}/*end if*/
